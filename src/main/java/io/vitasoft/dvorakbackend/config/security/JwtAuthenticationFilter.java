...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        String refreshToken = jwtTokenProvider.extractRefreshToken(request)
                .filter(jwtTokenProvider::isTokenValid) // filter()를 통해 유효한 RefreshToken을 반환합니다.
                .orElse(null); // RefreshToken이 유효하지 않거나 존재하지 않는다면 null을 반환합니다.
     ...
     *  그 후 JwtTokenProvider.sendAccessTokenAndRefreshToken()으로 응답 헤더에 보내기
     */
    public void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken) {
        memberRepository.findByRefreshToken(refreshToken)
                .ifPresent(member -> {
                    String reIssuedRefreshToken = reIssueRefreshToken(member);
                    jwtTokenProvider.sendAccessAndRefreshToken(response, jwtTokenProvider.createAccessToken(member.getEmail()),reIssuedRefreshToken);
                });
    }

    /**
     * 리프레시 토큰 재발급 & DB에 리프레시 토큰 업데이트 메서드
     * jwtTokenProvider.createRefreshToken()으로 리프레시 토큰 재발급 후, DB에 재발급한 리프레시 토큰 업데이트 후 Flush
     */
     ...
     * 인증 허가 처리된 객체를 SecurityContextHolder에 담기 (참고로 SecurityContextHolder는 인증된 사용자를 저장하는 객체)
     * 그 후 다음 인증 필터로 진행
     */
